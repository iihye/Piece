...
public interface HeartRepository extends JpaRepository<Piecesheart, Long> {
    ...
    @Query("select count(h) from Piecesheart h where h.user.userId = :userId and h.piece.pieceId = :pieceId")
    Integer findByPieceIdAndUserId(Long pieceId, Long userId);

    @Query("select h.heartId from Piecesheart h where h.user.userId = :userId and h.piece.pieceId = :pieceId")
