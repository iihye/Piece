...
export const useCakeDetailStore = defineStore("cakedetail", () => {
  ...
  const fetchHeartCount = async (cultureId) => {
    try {
      const response = await axios.get(
        ...
        }/cultures/heart/count/${cultureId}`
      );
  ...
  const findCultureId = async (cultureId) => {
    ...
    try {
      ...
      console.log("존재하는 오픈채팅방 번호?:" + chatRoomId.value);
  ...
  const makeOpenchat = async function (cultureId) {
    await axios({
  ...
  // 채팅 참여하기
  const joinChatRoom = async function () {
    console.log(chatRoomId.value + "번 채팅방 참여하기");
    await axios({
